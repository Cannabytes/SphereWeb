...
class Router {
    ...
    /**
     * Store a before middleware route and a handling function to be executed when accessed using one of the specified
     * methods.
     *
    ...
    public function getRequestMethod() {
        ...
        elseif($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if(isset($headers['X-HTTP-Method-Override']) && in_array($headers['X-HTTP-Method-Override'], [
                    'PUT',
                    'DELETE',
                    'PATCH',
                ])) {
     ...
     */
    public function patternMatches($pattern, $uri, &$matches, $flags) {
        // Replace all curly braces matches {} into word patterns (like Laravel)
    ...
    private function handle($routes, $quitAfterRun = false) {
        ...
        foreach($routes as $route) {
            ...
            if($is_match['ok']) {
                ...
                $this->invoke($route['fn'], $is_match['params']);
                ++$numHandled;
