...
class start_player_pack
{
    ...
    public function buy()
    {
        ...
        try {
            ...
            // Получаем и блокируем баланс пользователя
            $stmt = sql::prepare("SELECT donate_point FROM users WHERE id = ? FOR UPDATE");
            $stmt->execute([auth::get_id()]);
            $userData = $stmt->fetch();
            if (!$userData) {
                throw new Exception("Пользователь не найден");
            }
            $donatePoints = $userData['donate_point'];
            ...
            if ($db && $db->inTransaction()) {
                $db->rollBack(); // Откат транзакции, если она активна
            }
